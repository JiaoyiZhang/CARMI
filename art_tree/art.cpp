...
static void recursive_scan(art_node *n, std::vector<ValueT> &rets, int len)
{
    // Handle base cases
    if (!n)
        return;
    ...
    case NODE4:
        for (int i = 0; i < n->num_children; i++)
        {
            if (rets.size() >= len)
                break;
            recursive_scan(((art_node4 *)n)->children[i], rets, len);
        }
        break;
    ...
    case NODE16:
        for (int i = 0; i < n->num_children; i++)
        {
            if (rets.size() >= len)
                break;
            recursive_scan(((art_node16 *)n)->children[i], rets, len);
        }
        break;
    ...
    case NODE48:
        for (int i = 0; i < 256; i++)
        {
            ...
            recursive_scan(((art_node48 *)n)->children[idx - 1], rets, len);
        }
        break;
    ...
    case NODE256:
        for (int i = 0; i < 256; i++)
        {
            ...
            recursive_scan(((art_node256 *)n)->children[i], rets, len);
        }
        break;
