...
class PiecewiseLR {
 public:
  ...
  void Train(const carmi_params::TestDataVecType &dataset, int len) {
    ...
    if (size == 0) return;
    carmi_params::TestDataVecType data(size, {DBL_MIN, DBL_MIN});
    ...
    int cand_size = 1000;
    carmi_params::TestDataVecType cand_point(cand_size, {0, 0});
    ...
    for (int j = 1; j < cand_size; j++) {
      dp[0][j].cost = cand_cost.CalculateCost(0, j, cand_index[j],
                                              cand_point[0], cand_point[j]);
      ...
      for (int j = i + 1; j < cand_size - 1; j++) {
        ...
        for (int k = i; k < j; k++) {
          ...
          if (res < opt.cost) {
            ...
            float var0 = cand_cost.Diff(i + 1, len, dp_idx);
            float var1 = cand_cost.Diff(i + 1, len, opt.idx);
            if (var0 < var1) {
              for (int l = 0; l < i; l++) {
                opt.idx[l] = dp[0][k].idx[l];
                opt.key[l] = dp[0][k].key[l];
    ...
    for (int j = 7; j < cand_size; j++) {
      if (dp[1][j].cost < opt.cost) {
        ...
        dp[1][j].idx[7] = length;
        opt.idx[7] = length;
        float var0 = cand_cost.Diff(8, len, dp[1][j].idx);
        float var1 = cand_cost.Diff(8, len, opt.idx);
        if (var0 < var1) {
          opt.cost = dp[1][j].cost;
          for (int k = 0; k < 7; k++) {
            opt.idx[k] = dp[1][j].idx[k];
