...
inline void CARMI::initLR(LRModel *lr,
...
inline void LRModel::Train(const std::vector<std::pair<double, double>> &dataset) {
  int actualSize = 0;
  ...
  divisor = static_cast<float>(maxValue - minValue) / 6;
  ...
  for (int k = 1; k <= 6; k++) {
    ...
    for (; i < dataset.size() - 1; i++) {
      if (dataset[i].first != -1) {
        if (static_cast<float>(dataset[i].first - minValue) / divisor >= k)
          break;
        cnt++;
