...
double CARMI::CalLeafFindTime(int actualSize, double density, const TYPE &node, const DataRange &range) const
{
    double time = 0;
 ...
 */
template <typename TYPE>
double CARMI::CalLeafInsertTime(int actualSize, double density, const TYPE &node, const DataRange &range, const DataRange &findRange) const
{
    double time = 0;
    for (int i = range.left; i < range.left + range.size; i++)
    {
...
NodeCost CARMI::dpLeaf(const IndexPair &dataRange)
{
    ...
    CalLeafInsertTime<ArrayType>(actualSize, 1, tmp, dataRange.insertRange, dataRange.findRange);
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        CalLeafFindTime<GappedArrayType>(actualSize, Density[i], tmpNode, dataRange.findRange);
        CalLeafInsertTime<GappedArrayType>(actualSize, Density[i], tmpNode, dataRange.insertRange, dataRange.findRange);
