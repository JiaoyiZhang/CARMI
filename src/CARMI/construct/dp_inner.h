...
void CARMI::CalInner(int c, int type, int frequency, double time, const IndexPair &dataRange, NodeCost *optimalCost, ParamStruct *optimalStruct)
{
...
NodeCost CARMI::dpInner(const IndexPair &dataRange)
{
    ...
    for (int c = 2; c < tmpEnd; c *= 2)
    {
        ...
        if (c * 512 < dataRange.initRange.size)
            continue;
#endif // DEBUG
        CalInner<LRModel>(c, 0, frequency, LRInnerTime, dataRange, &optimalCost, &optimalStruct);
        CalInner<PLRModel>(c, 1, frequency, PLRInnerTime, dataRange, &optimalCost, &optimalStruct);
        if (c <= 160)
            CalInner<HisModel>(c, 2, frequency, HisInnerTime, dataRange, &optimalCost, &optimalStruct);
        if (c <= 20)
            CalInner<BSModel>(c, 3, frequency, BSInnerTime, dataRange, &optimalCost, &optimalStruct);
    }
