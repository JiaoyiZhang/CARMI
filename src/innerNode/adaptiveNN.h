#ifndef ADAPTIVE_NN_H
#define ADAPTIVE_NN_H

#include "innerNode.h"
#include "../trainModel/lr.h"

template <typename lowerType>
class adaptiveNN : public basicInnerNode
{
public:
    adaptiveNN() : basicInnerNode(){};
    adaptiveNN(params firstStageParams, params secondStageParams, int maxKey, int childNum, int cap) : basicInnerNode(childNum)
    {
...
private:
    ...
    net m_firstStageNetwork = net(); // lr of the first stage
};
...
void adaptiveNN<lowerType>::init(const vector<pair<double, double>> &dataset)
{
    ...
    for (int i = 0; i < childNumber; i++)
    {
        if (perSubDataset[i].size() > maxKeyNum)
        {
            ...
            adaptiveNN *child = new adaptiveNN(m_firstStageParams, m_secondStageParams, maxKeyNum, childNumber, capacity);
...
bool adaptiveNN<lowerType>::insert(pair<double, double> data)
{
    ...
    if (children[preIdx]->isLeaf() && size >= maxKeyNum)
    {
        ...
        adaptiveNN *newNode = new adaptiveNN(m_firstStageParams, m_secondStageParams, maxKeyNum, childNumber, capacity);
        ...
        return ((adaptiveNN *)children[preIdx])->insert(data);
    }
...
template <typename lowerType>
bool adaptiveNN<lowerType>::del(double key)
{
    double p = m_firstStageNetwork.predict(key);
    int preIdx = static_cast<int>(p * (childNumber - 1));
    if (children[preIdx]->isLeaf() == false)
        return ((adaptiveNN *)children[preIdx])->del(key);
    return children[preIdx]->del(key);
}

template <typename lowerType>
bool adaptiveNN<lowerType>::update(pair<double, double> data)
{
    double p = m_firstStageNetwork.predict(data.first);
    int preIdx = static_cast<int>(p * (childNumber - 1));
    if (children[preIdx]->isLeaf() == false)
        return ((adaptiveNN *)children[preIdx])->update(data);
