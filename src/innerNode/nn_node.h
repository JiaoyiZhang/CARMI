...
public:
    ...
    AdaptiveNN(int childNum) : NetworkNode(childNum)
    {
        model = new Net();
    }
...
bool AdaptiveNN::Insert(pair<double, double> data)
{
    ...
    if (this->children_is_leaf[preIdx] == true)
    {
        ...
        if (size >= kMaxKeyNum)
        {
            ...
            AdaptiveNN *newNode = (AdaptiveNN *)InnerNodeCreator(kInnerNodeID, kAdaptiveChildNum);
...
long double AdaptiveNN::GetCost(const btree::btree_map<double, pair<int, int>> &cntTree, int childNum, const vector<pair<double, double>> &dataset)
{
    ...
    for (int i = 0; i < childNum; i++)
    {
        if (perSubDataset[i].size() > kMaxKeyNum)
            totalCost += AdaptiveNN::GetCost(cntTree, childNum, perSubDataset[i]);
        else
            totalCost += LEAF_NODE_TYPE::GetCost(cntTree, perSubDataset[i]);
    }
    cout << "sub tree get cost finish!" << endl;
